...
public interface IReadRepository<T> where T : EntidadeBase
{
    ...
    Task<PagedResult<T>> ObterPaginadoAsync(int pagina, int tamanhoPagina, Expression<Func<T, bool>>? predicates, Func<IQueryable<T>, IOrderedQueryable<T>>? orderBy, params Expression<Func<T, object>>[] includes);
}
